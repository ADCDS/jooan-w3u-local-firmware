...
case "$jl_action" in
    prepare|validate)
        ...
        gzip -t "$jl_source/controller.tar.gz" || exit 1
        ...
        jl_wait_free_kb "$JL_ROOT" $(( (jl_peak + 1023) / 1024 + 80 )) 20 || exit 1
        ...
        gzip -t "$JL_ROOT/controller.tar.gz" || exit 1
    ...
    activate)
        ...
        jl_check_current_storage || exit 1
        jl_wait_free_kb "$JL_ROOT" 84 20 || exit 1
