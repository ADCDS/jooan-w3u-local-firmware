...
typedef struct { unsigned char *data;size_t len;uint32_t sequence; } Fragment;
typedef struct {
...
static int append_nal(Stream*s,const unsigned char*p,size_t n)
{
    ...
    if(type==7&&n<=sizeof(s->sps)){memcpy(s->sps,p,n);s->sps_len=n;if(s->pps_len&&!s->init)build_init(s);}
    if(type==8&&n<=sizeof(s->pps)){memcpy(s->pps,p,n);s->pps_len=n;if(s->sps_len&&!s->init)build_init(s);}
...
static void publish_gop(Stream*s)
{
    ...
    if(!s->sample_count||!s->gop_len)return;
    ...
    pthread_mutex_lock(&s->lock);f=&s->ring[s->ring_next++%RING];free(f->data);f->data=b.p;f->len=b.n;f->sequence=s->sequence;pthread_cond_broadcast(&s->changed);pthread_mutex_unlock(&s->lock);
}
...
int joan_fmp4_fragment(const char*id,uint32_t after,unsigned char**data,size_t*len,uint32_t*seq,unsigned timeout){Stream*s=find_stream(id);Fragment*best=NULL;unsigned i;if(!s||!data||!len||!seq)return-1;pthread_mutex_lock(&s->lock);for(;;){for(i=0;i<RING;i++)if(s->ring[i].sequence>after&&(!best||s->ring[i].sequence<best->sequence))best=&s->ring[i];if(best)break;if(timedwait(s,timeout))break;}if(!best){pthread_mutex_unlock(&s->lock);return-1;}*data=malloc(best->len);if(!*data){pthread_mutex_unlock(&s->lock);return-1;}memcpy(*data,best->data,best->len);*len=best->len;*seq=best->sequence;pthread_mutex_unlock(&s->lock);return 0;}
