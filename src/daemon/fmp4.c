...
typedef struct { unsigned char *data;size_t len;uint32_t sequence;time_t published; } Fragment;
typedef struct {
    ...
    Fragment ring[RING];unsigned ring_next;
} Stream;
...
static int grow(Buf*b,size_t add){size_t need=b->n+add,cap=b->cap?b->cap:1024;unsigned char*p;if(need<=b->cap)return 0;while(cap<need){if(cap>8u*1024u*1024u)return-1;cap*=2;}p=realloc(b->p,cap);if(!p)return-1;b->p=p;b->cap=cap;return 0;}
static int put(Buf*b,const void*p,size_t n){if(grow(b,n))return-1;memcpy(b->p+b->n,p,n);b->n+=n;return 0;}
...
static size_t box(Buf*b,const char*t){size_t p=b->n;u32(b,0);put(b,t,4);return p;}static void endbox(Buf*b,size_t p){uint32_t z=(uint32_t)(b->n-p);b->p[p]=z>>24;b->p[p+1]=z>>16;b->p[p+2]=z>>8;b->p[p+3]=z;}
...
static int append_nal(Stream*s,const unsigned char*p,size_t n)
{
    ...
    if(type==7&&n<=sizeof(s->sps)){if(s->init&&(s->sps_len!=n||memcmp(s->sps,p,n))){s->sps_len=s->pps_len=0;return-1;}memcpy(s->sps,p,n);s->sps_len=n;if(s->pps_len&&!s->init)build_init(s);}
    if(type==8&&n<=sizeof(s->pps)){if(s->init&&(s->pps_len!=n||memcmp(s->pps,p,n))){s->sps_len=s->pps_len=0;return-1;}memcpy(s->pps,p,n);s->pps_len=n;if(s->sps_len&&!s->init)build_init(s);}
...
static void publish_gop(Stream*s)
{
    Buf b={0};size_t moof,mfhd,traf,tfhd,tfdt,trun,offset_pos,mdat,i;uint32_t data_offset;Fragment*f;
    if(!s->init||!s->sample_count||!s->gop_len)return;
    moof=box(&b,"moof");mfhd=box(&b,"mfhd");u32(&b,0);u32(&b,++s->sequence);endbox(&b,mfhd);traf=box(&b,"traf");tfhd=box(&b,"tfhd");u32(&b,0x020000);u32(&b,1);endbox(&b,tfhd);tfdt=box(&b,"tfdt");u32(&b,0x01000000);u64(&b,s->decode_time);endbox(&b,tfdt);trun=box(&b,"trun");u32(&b,0x00000701);u32(&b,(uint32_t)s->sample_count);offset_pos=b.n;u32(&b,0);for(i=0;i<s->sample_count;i++){u32(&b,s->samples[i].duration);u32(&b,s->samples[i].size);u32(&b,s->samples[i].flags);s->decode_time+=s->samples[i].duration;}endbox(&b,trun);endbox(&b,traf);endbox(&b,moof);data_offset=(uint32_t)b.n+8;b.p[offset_pos]=data_offset>>24;b.p[offset_pos+1]=data_offset>>16;b.p[offset_pos+2]=data_offset>>8;b.p[offset_pos+3]=data_offset;mdat=box(&b,"mdat");put(&b,s->gop,s->gop_len);endbox(&b,mdat);
    pthread_mutex_lock(&s->lock);f=&s->ring[s->ring_next++%RING];free(f->data);f->data=b.p;f->len=b.n;f->sequence=s->sequence;f->published=time(NULL);pthread_cond_broadcast(&s->changed);pthread_mutex_unlock(&s->lock);
}

static void finish_access(Stream*s,uint32_t timestamp)
{
    uint32_t duration=s->last_timestamp?timestamp-s->last_timestamp:6000;size_t i;
    if(!s->access_len)return;
    if(!duration||duration>RTP_TIMESCALE)duration=6000;
    ...
    if(s->access_idr&&s->sample_count){publish_gop(s);s->gop_len=0;s->sample_count=0;}
    if(!s->sample_count&&!s->access_idr){s->access_len=0;s->access_idr=0;s->last_timestamp=timestamp;return;}
    if(s->sample_count>=MAX_SAMPLES||s->gop_len+s->access_len>MAX_GOP){s->gop_len=s->sample_count=0;goto drop;}
...
drop:s->access_len=0;s->access_idr=0;s->last_timestamp=timestamp;
}
...
static void reset_stream(Stream*s)
{
    ...
    s->ring_next=0;s->sequence+=RING+1;
    pthread_cond_broadcast(&s->changed);
    pthread_mutex_unlock(&s->lock);
    s->sps_len=s->pps_len=0;s->access_len=0;s->access_idr=0;
    s->gop_len=s->sample_count=0;s->last_timestamp=0;s->decode_time=0;
}

static int run_stream(Stream*s)
{
    int fd=-1;struct sockaddr_in a;char url[256],control[512],reply[16384],session[128]="",extra[512],password[320],password_path[512];unsigned char packet[MAX_NAL+64],fu[MAX_NAL],*secret=NULL;size_t fu_len=0,secret_len=0;uint32_t fu_ts=0;uint16_t previous_rtp_seq=0;int have_rtp_seq=0,damaged=0;Digest digest;
    ...
    for(;;){unsigned char ch,channel;uint16_t plen;size_t off;uint32_t ts;int marker;unsigned type;if(recv(fd,&ch,1,MSG_WAITALL)!=1)goto fail;if(ch!='$')continue;if(recv(fd,packet,3,MSG_WAITALL)!=3)goto fail;channel=packet[0];plen=((uint16_t)packet[1]<<8)|packet[2];if(recv(fd,packet,plen,MSG_WAITALL)!=(ssize_t)plen)goto fail;if(channel!=0)continue;if(plen<12||(packet[0]&0xc0)!=0x80)continue;{uint16_t current=((uint16_t)packet[2]<<8)|packet[3];if(have_rtp_seq&&(uint16_t)(previous_rtp_seq+1)!=current){damaged=1;fu_len=0;s->access_len=0;s->access_idr=0;s->gop_len=s->sample_count=0;}previous_rtp_seq=current;have_rtp_seq=1;}off=12+(packet[0]&15u)*4u;if(packet[0]&0x10){if(off+4>plen)continue;off+=4+(((size_t)packet[off+2]<<8)|packet[off+3])*4;}if(off>=plen)continue;marker=(packet[1]&0x80)!=0;ts=((uint32_t)packet[4]<<24)|((uint32_t)packet[5]<<16)|((uint32_t)packet[6]<<8)|packet[7];type=packet[off]&31u;
        ...
        if(marker){if(damaged){s->access_len=0;s->access_idr=0;s->last_timestamp=ts;damaged=0;}else finish_access(s,ts);}
    }
...
int joan_fmp4_init_segment(const char*id,unsigned char**data,size_t*len,unsigned timeout){Stream*s=find_stream(id);if(!s||!data||!len)return-1;pthread_mutex_lock(&s->lock);while(!s->init_len&&!timedwait(s,timeout)){}if(!s->init_len||strcmp(s->status,"connected")){pthread_mutex_unlock(&s->lock);return-1;}*data=malloc(s->init_len);if(!*data){pthread_mutex_unlock(&s->lock);return-1;}memcpy(*data,s->init,s->init_len);*len=s->init_len;pthread_mutex_unlock(&s->lock);return 0;}
int joan_fmp4_fragment(const char*id,uint32_t after,unsigned char**data,size_t*len,uint32_t*seq,unsigned timeout){Stream*s=find_stream(id);Fragment*best;unsigned i;int rc=-1;if(!s||!data||!len||!seq)return-1;pthread_mutex_lock(&s->lock);for(;;){best=NULL;for(i=0;i<RING;i++)if(s->ring[i].data&&s->ring[i].sequence>after&&(!best||(after? s->ring[i].sequence<best->sequence:s->ring[i].sequence>best->sequence)))best=&s->ring[i];if(best){/* Old retained video is never a live view, even if the RTSP socket has not timed out yet. */if(strcmp(s->status,"connected")||time(NULL)-best->published>8){rc=-2;break;}if(after&&best->sequence!=after+1){rc=-2;break;}*data=malloc(best->len);if(!*data)break;memcpy(*data,best->data,best->len);*len=best->len;*seq=best->sequence;rc=0;break;}if(after&&s->sequence>after&&s->sequence-after>RING){rc=-2;break;}if(after&&s->sequence<after&&s->sequence){rc=-2;break;}if(timedwait(s,timeout))break;}pthread_mutex_unlock(&s->lock);return rc;}
