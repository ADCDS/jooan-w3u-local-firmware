...
static int parse_request(Conn*c,JoanRequest*r){unsigned char*h=malloc(JOAN_MAX_HEADERS+1),*end;size_t used=0,head;char*line_end,*headers,*q;ssize_t n;if(!h)return-1;memset(r,0,sizeof(*r));r->fd=c->fd;while(used<JOAN_MAX_HEADERS){n=cread(c,h+used,JOAN_MAX_HEADERS-used);if(n<=0)goto fail;used+=(size_t)n;h[used]=0;end=(unsigned char*)strstr((char*)h,"\r\n\r\n");if(end)break;}if(!end)goto fail;head=(size_t)(end-h)+4;line_end=strstr((char*)h,"\r\n");if(!line_end)goto fail;*line_end=0;headers=line_end+2;if(sscanf((char*)h,"%11s %511s",r->method,r->path)!=2)goto fail;q=strchr(r->path,'?');if(q){*q++=0;snprintf(r->query,sizeof(r->query),"%s",q);}header_copy(headers,"Host",r->host,sizeof(r->host));header_copy(headers,"Cookie",r->cookie,sizeof(r->cookie));header_copy(headers,"X-CSRF-Token",r->csrf,sizeof(r->csrf));header_copy(headers,"Content-Type",r->content_type,sizeof(r->content_type));header_copy(headers,"Upgrade",r->upgrade,sizeof(r->upgrade));header_copy(headers,"Sec-WebSocket-Key",r->ws_key,sizeof(r->ws_key));header_copy(headers,"Sec-WebSocket-Protocol",r->ws_protocol,sizeof(r->ws_protocol));header_copy(headers,"Origin",r->origin,sizeof(r->origin));header_copy(headers,"Transfer-Encoding",r->transfer_encoding,sizeof(r->transfer_encoding));if(!r->host[0]||r->transfer_encoding[0])goto fail;{char cl[32]={0};if(!header_copy(headers,"Content-Length",cl,sizeof(cl))){char*ep=NULL;unsigned long z=strtoul(cl,&ep,10);if(!ep||*ep)goto fail;r->content_length=(size_t)z;}}if(r->content_length>JOAN_MAX_BODY)goto fail;if(r->content_length){size_t have=used-head,off=0;r->body=malloc(r->content_length+1);if(!r->body)goto fail;if(have>r->content_length)have=r->content_length;memcpy(r->body,end+4,have);off=have;while(off<r->content_length){n=cread(c,r->body+off,r->content_length-off);if(n<=0)goto fail;off+=(size_t)n;}r->body[r->content_length]=0;}free(h);return 0;fail:free(r->body);free(h);return-1;}
...
static int route(Conn*c,JoanRequest*r){JoanAuthz a;char x[256],y[256],id[65],path[512],body[1536];
    ...
    if(!strcmp(r->path,"/api/v1/setup/status")){
        snprintf(body,sizeof(body),"{\"state\":\"ready\",\"setup_required\":false,\"default_password_warning\":%s,\"temporary_user\":\"admin\"}",joan_auth_setup_required(&G)?"true":"false");
    ...
    if(!strcmp(r->path,"/api/status")){unsigned char*routes=NULL;size_t routes_len=0;const char*routes_json="[]";if(!joan_run_helper(&G,"routes-list",NULL,NULL,&routes,&routes_len,2)&&routes_len&&routes_len<512&&(routes[0]=='['||routes[0]=='{'))routes_json=(char*)routes;snprintf(body,sizeof(body),"{\"version\":\"%s\",\"state\":\"ready\",\"local_only\":true,\"active_routes\":%s,\"default_password_warning\":%s,\"ssh_password_sync\":%s,\"rtsp_password_sync\":%s,\"features\":{\"https_identity\":%s,\"firewall\":false,\"rtsp\":true,\"fmp4_main\":\"%s\",\"fmp4_sub\":\"%s\",\"ptz\":\"helper-gated\",\"audio\":\"socket-gated\",\"ssh\":\"password-synchronized\",\"mdns\":\"%s\"},\"mqtt_bridge\":\"%s\",\"https_cloud_bridge\":{\"supported\":false,\"blocker\":\"OEM trust/pinning contract not recovered\"}}",JOAN_VERSION,routes_json,a.must_change?"true":"false",joan_auth_ssh_synchronized(&G)?"true":"false",joan_auth_rtsp_synchronized(&G)?"true":"false",G.plain_http?"false":"true",joan_fmp4_status("main"),joan_fmp4_status("sub"),joan_mdns_status(),joan_mqtt_bridge_status());free(routes);return json(c,200,body);}
...
static void serve(Conn*c,const char*remote){JoanRequest r;if(parse_request(c,&r)){error_json(c,400,"bad_request","malformed request, transfer encoding, or oversized body");return;}snprintf(r.remote,sizeof(r.remote),"%s",remote);if(!strncmp(r.path,"/api/",5)||!strncmp(r.path,"/ws/",4))route(c,&r);else static_file(c,r.path);if(r.body){memset(r.body,0,r.content_length);free(r.body);}}
