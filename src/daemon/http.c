 ...
 * Used by the microSD recording curator, which reads/manages the OEM recorder's
 * on-card files rather than owning recording itself. */
...
static int recording_day_ok(const char*s){int i;for(i=0;i<8;i++)if(s[i]<'0'||s[i]>'9')return 0;return s[8]==0;}
static int recording_name_ok(const char*s){size_t i,n=strlen(s);if(!n||n>127||strstr(s,".."))return 0;for(i=0;i<n;i++){char ch=s[i];if(!((ch>='0'&&ch<='9')||(ch>='A'&&ch<='Z')||(ch>='a'&&ch<='z')||ch=='.'||ch=='_'||ch=='-'))return 0;}return 1;}
/* Rewrite one recorded clip as MP4 on the way out so a browser can play it.
 * The OEM records AVI, which no browser opens; nothing is re-encoded. */
static int mp4_sink(void*ctx,const void*data,size_t len){return wall((Conn*)ctx,data,len);}
static int play_recording(Conn*c,const char*day,const char*name,const char*range){char p[600],hdr[520];JoanRecMp4*m;long total,from=-1,to=-1;int hn,rc,partial=0;
    if(snprintf(p,sizeof(p),"%s/JOOAN_RECORD/%s/%s",G.sd_dir,day,name)>=(int)sizeof(p))return error_json(c,404,"recording_not_found","recording not found");
    m=joan_recmp4_open(p);
    if(!m)return error_json(c,415,"recording_not_playable","this clip cannot be prepared for playback; download it instead");
    total=joan_recmp4_length(m);
    /* A browser seeks with Range; without it the clip plays but will not scrub. */
    if(range&&!strncasecmp(range,"bytes=",6)){char*ep=NULL;const char*q=range+6;long a=strtol(q,&ep,10);
        if(ep&&ep!=q){from=a;if(*ep=='-'&&ep[1]){char*e2=NULL;long b=strtol(ep+1,&e2,10);if(e2&&e2!=ep+1)to=b;}partial=1;}
        else if(ep&&*ep=='-'){long b=strtol(ep+1,NULL,10);if(b>0&&b<=total){from=total-b;to=total-1;partial=1;}}}
    if(partial){if(from<0)from=0;if(to<0||to>=total)to=total-1;
        if(from>=total||from>to){char cr[64];snprintf(cr,sizeof(cr),"Content-Range: bytes */%ld\r\n",total);joan_recmp4_close(m);return response(c,416,"application/json","",0,cr);}}
    hn=partial
        ?snprintf(hdr,sizeof(hdr),"HTTP/1.1 206 Partial Content\r\nContent-Type: video/mp4\r\nContent-Length: %ld\r\nContent-Range: bytes %ld-%ld/%ld\r\nAccept-Ranges: bytes\r\nConnection: close\r\nX-Content-Type-Options: nosniff\r\nCache-Control: no-store\r\n%s\r\n",to-from+1,from,to,total,G.plain_http?"":"Strict-Transport-Security: max-age=31536000\r\n")
        :snprintf(hdr,sizeof(hdr),"HTTP/1.1 200 OK\r\nContent-Type: video/mp4\r\nContent-Length: %ld\r\nAccept-Ranges: bytes\r\nConnection: close\r\nX-Content-Type-Options: nosniff\r\nCache-Control: no-store\r\n%s\r\n",total,G.plain_http?"":"Strict-Transport-Security: max-age=31536000\r\n");
    if(hn<=0||(size_t)hn>=sizeof(hdr)||wall(c,hdr,(size_t)hn)){joan_recmp4_close(m);return -1;}
    rc=joan_recmp4_write(m,mp4_sink,c,partial?from:-1,partial?to:-1);joan_recmp4_close(m);return rc;}

/* Stream one recorded clip straight off the card. Files can be hundreds of MB,
 * so this never buffers the whole file the way joan_read_file would. */
static int stream_recording(Conn*c,const char*day,const char*name){char p[600],hdr[700],disp[256];struct stat st;int fd,hn;unsigned char buf[16384];ssize_t z;if(snprintf(p,sizeof(p),"%s/JOOAN_RECORD/%s/%s",G.sd_dir,day,name)>=(int)sizeof(p))return error_json(c,404,"recording_not_found","recording not found");fd=open(p,O_RDONLY);if(fd<0)return error_json(c,404,"recording_not_found","recording not found");if(fstat(fd,&st)||!S_ISREG(st.st_mode)){close(fd);return error_json(c,404,"recording_not_found","recording not found");}snprintf(disp,sizeof(disp),"Content-Disposition: attachment; filename=\"%s-%s.avi\"\r\n",day,name);hn=snprintf(hdr,sizeof(hdr),"HTTP/1.1 200 OK\r\nContent-Type: video/x-msvideo\r\nContent-Length: %lld\r\nConnection: close\r\nX-Content-Type-Options: nosniff\r\nCache-Control: no-store\r\n%s%s\r\n",(long long)st.st_size,G.plain_http?"":"Strict-Transport-Security: max-age=31536000\r\n",disp);if(hn<=0||(size_t)hn>=sizeof(hdr)||wall(c,hdr,(size_t)hn)){close(fd);return -1;}while((z=read(fd,buf,sizeof(buf)))>0)if(wall(c,buf,(size_t)z)){close(fd);return -1;}close(fd);return 0;}
...
static int route(Conn*c,JoanRequest*r){JoanAuthz a;char x[256],y[256],id[65],path[512],body[1536];
    ...
    if(!strncmp(r->path,"/api/video/",11)&&!strcmp(r->method,"GET")){char stream[16],kind[32];unsigned char*out=NULL;size_t n=0;uint32_t seq=0,first=0,after=0;char extra[96];if(sscanf(r->path,"/api/video/%15[^/]/%31s",stream,kind)!=2|| (strcmp(stream,"main")&&strcmp(stream,"sub")))return error_json(c,404,"stream_not_found","unknown stream");if(!strcmp(kind,"init.mp4")){if(joan_fmp4_init_segment(stream,&out,&n,5000))return error_json(c,503,"stream_unavailable","initialization segment is not ready");response(c,200,"video/mp4",out,n,NULL);free(out);return 0;}if(strcmp(kind,"fragment.mp4"))return error_json(c,404,"route_not_found","unknown video route");if(r->query[0]){char*ep=NULL;if(strncmp(r->query,"after=",6))return error_json(c,400,"invalid_sequence","expected after sequence");after=(uint32_t)strtoul(r->query+6,&ep,10);if(!ep||*ep)return error_json(c,400,"invalid_sequence","expected numeric sequence");}{int fragment_result=joan_fmp4_fragment(stream,after,&out,&n,&first,&seq,10000);if(fragment_result==-2)return error_json(c,409,"stream_discontinuity","video timeline changed; reload initialization segment");if(fragment_result)return error_json(c,503,"stream_timeout","no new fragment is available");}snprintf(extra,sizeof(extra),"X-Joan-First-Sequence: %u\r\nX-Joan-Sequence: %u\r\n",first,seq);response(c,200,"video/mp4",out,n,extra);free(out);return 0;}
    ...
    /* ---- microSD storage + recordings (OEM recorder curator) ---- */
    if(!strcmp(r->path,"/api/v1/storage/sd")&&!strcmp(r->method,"GET"))return helper_passthrough(c,"sd-status",NULL,NULL);
    if(!strcmp(r->path,"/api/v1/recordings/days")&&!strcmp(r->method,"GET"))return helper_passthrough(c,"recordings-days",NULL,NULL);
    if(!strncmp(r->path,"/api/v1/recordings/day/",23)){char day[16];snprintf(day,sizeof(day),"%s",r->path+23);if(!recording_day_ok(day))return error_json(c,404,"recording_not_found","unknown day");if(!strcmp(r->method,"GET"))return helper_passthrough(c,"recordings-list",NULL,day);if(!strcmp(r->method,"DELETE"))return helper_passthrough(c,"recordings-delete",NULL,day);return error_json(c,404,"route_not_found","route not found");}
    if(!strncmp(r->path,"/api/v1/recordings/play/",24)){char day[16],name[128];if(sscanf(r->path+24,"%15[0-9]/%127s",day,name)!=2||!recording_day_ok(day)||!recording_name_ok(name))return error_json(c,404,"recording_not_found","unknown recording");if(strcmp(r->method,"GET"))return error_json(c,404,"route_not_found","route not found");return play_recording(c,day,name,r->range);}
    if(!strncmp(r->path,"/api/v1/recordings/file/",24)){char day[16],name[128],ref[160];if(sscanf(r->path+24,"%15[0-9]/%127s",day,name)!=2||!recording_day_ok(day)||!recording_name_ok(name))return error_json(c,404,"recording_not_found","unknown recording");if(!strcmp(r->method,"GET"))return stream_recording(c,day,name);if(!strcmp(r->method,"DELETE")){snprintf(ref,sizeof(ref),"%s/%s",day,name);return helper_passthrough(c,"recordings-delete",NULL,ref);}return error_json(c,404,"route_not_found","route not found");}
