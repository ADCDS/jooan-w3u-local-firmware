...
static int route(Conn*c,JoanRequest*r){JoanAuthz a;char x[256],y[256],id[65],path[512],body[1536];
    ...
    if(!strcmp(r->path,"/api/status")){snprintf(body,sizeof(body),"{\"version\":\"%s\",\"state\":\"%s\",\"must_change\":%s,\"features\":{\"https_identity\":%s,\"firewall\":false,\"rtsp\":true,\"ptz\":\"helper-gated\",\"audio\":\"socket-gated\",\"ssh\":\"helper-gated\",\"mdns\":\"%s\"},\"mqtt_bridge\":\"%s\",\"https_cloud_bridge\":{\"supported\":false,\"blocker\":\"OEM trust/pinning contract not recovered\"}}",JOAN_VERSION,a.must_change?"setup-required":"ready",a.must_change?"true":"false",G.plain_http?"false":"true",joan_mdns_status(),joan_mqtt_bridge_status());return json(c,200,body);}
    ...
    if(!strcmp(r->path,"/api/mdns")&&(!strcmp(r->method,"PUT")||!strcmp(r->method,"POST"))){if(json_field(r->body,r->content_length,"hostname",x,sizeof(x))||joan_mdns_set_hostname(&G,x))return json(c,400,"{\"error\":\"hostname must be a 1-63 character DNS label\"}");joan_mdns_get_hostname(&G,x);snprintf(body,sizeof(body),"{\"ok\":true,\"hostname\":\"%s\",\"address\":\"%s.local\"}",x,x);return json(c,200,body);}
