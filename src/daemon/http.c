...
static void serve(Conn*c,const char*remote){JoanRequest r;int parsed=parse_request(c,&r);if(parsed){if(parsed==-2)error_json(c,413,"payload_too_large","request body exceeds route limit");else error_json(c,400,"bad_request","malformed request or unsupported transfer encoding");return;}snprintf(r.remote,sizeof(r.remote),"%s",remote);if(!strncmp(r.path,"/api/",5)||!strncmp(r.path,"/ws/",4))route(c,&r);else static_file(c,r.path);if(r.body){memset(r.body,0,r.content_length);free(r.body);}}
